...
class WafEvidenceDirectorySource(BaseModel):
    type: Literal["directory"] = "directory"
    path: str


class WafEvidenceRequestV1(BaseModel):
    request_version: Literal["waf-evidence-request/v1"] = "waf-evidence-request/v1"
    task_id: str
    source: WafEvidenceDirectorySource
...
class WafEvidenceInputSummary(BaseModel):
...
class WafEvidenceResponseV1(BaseModel):
    response_version: Literal["waf-evidence-response/v1"] = "waf-evidence-response/v1"
    analyzer_version: str
    extraction_started_at: str
    extraction_finished_at: str
    input_summary: WafEvidenceInputSummary | None = None
    result: LogEvidenceV1
